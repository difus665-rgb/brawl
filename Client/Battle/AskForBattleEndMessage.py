...
from Server.Battle.BattleResultRoboWarsMessage import BattleResultRoboWarsMessage
from Server.Battle.BattleResultBigGameMessage import BattleResultBigGameMessage
from Server.Battle.BattleResultBossFightMessage import BattleResultBossFightMessage
from Server.Battle.BattleResultDebacleMessage import BattleResultDebacleMessage
from Server.Battle.BattleResultRoboRumbleMessage import BattleResultRoboRumbleMessage
import json
...
class AskForBattleEndMessage(BSMessageReader):
    ...
    def process(self):
        ...
        elif self.map in [97,98,99,131,142,264,335]:
            BattleResultRoboWarsMessage(self.client, self.player, 1).send()
        elif self.map in [21, 30]:
            BattleResultBigGameMessage(self.client, self.player, 1).send()
        elif self.map in [57]:
            BattleResultBossFightMessage(self.client, self.player, 1).send()
        elif self.map in [269]:
            BattleResultDebacleMessage(self.client, self.player, 1).send()
        elif self.map in [68]:
            BattleResultRoboRumbleMessage(self.client, self.player, 1).send()
        elif self.players == 6:
