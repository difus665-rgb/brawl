...
class BattleResultBigGameMessage(Writer):
    ...
    def encode(self):
        ...
        if self.player.battle_result == 1:
            tropGained = lose_val
            tokenGained = min(10, max(0, self.player.player_tokens))
        elif self.player.battle_result == 0:
            tropGained = win_val
            tokenGained = min(50, max(0, self.player.player_tokens))
            
        # Расчёт токенов
        tokenevent = tokenGained if self.double_tokens else 0
        totaltokens = tokenGained + tokenevent  # Базовые токены + бонус события
        doubledtokens = min(tokenGained, self.player.tokensdoubler)  # Удвоители добавляют бонус, равный tokenGained
        totaltokens2 = totaltokens + doubledtokens  # Итоговые токены с удвоителями
